...
function train_batch(roidbs)
    ...
    feval = function(x)
      ...
      for i = 1, #train_batch[1] do
        ...
        y_labels = output[1]:permute(2,3,1):cuda()
        ...
        y_reg = output[2]:permute(2,3,1)
        y_reg:resize(y_reg:size(1)*y_reg:size(2)*k, 4)
        y_reg[torch.ne(target_bbox_loss,1)] = 0
        
        local temp1 = torch.sum(target_bbox_loss) / 4
        local temp2 = y_reg[target_bbox_loss:eq(1)]:reshape(temp1, 4)
        local temp3 = target_reg[target_bbox_loss:eq(1)]:reshape(temp1, 4)
        print(temp2:cat(temp3,2))
        ...
        if (processedImages <= imgCount) then
          table.insert(save_images,mini_batch:clone():float())
      ...
      reg_correct = reg_correct + correct_boxes
        ...
        if err > 100 then
          print(sl1:forward(y[2], target[2]))
          print(log:forward(y[1], target[1]))
        end
