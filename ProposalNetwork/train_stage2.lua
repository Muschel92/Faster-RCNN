...
function check_error(labels, labels_weight, output)
    local acc_fg = torch.cmul(torch.gt(output[{{1},{1},{},{}}], output[{{1},{2},{},{}}]), torch.eq(labels, 1))
    local acc_bg = torch.cmul(torch.le(output[{{1},{1},{},{}}], output[{{1},{2},{},{}}]), torch.eq(labels, 2))
    
    acc_fg = torch.sum(torch.cmul(acc_fg:float():cuda(), labels_weight)) / (torch.sum(labels_weight[torch.eq(labels, 1)]))
    if(torch.sum(torch.eq(labels,2)) > 0) then
      acc_bg = torch.sum(torch.cmul(acc_bg:float():cuda(), labels_weight)) / (torch.sum(labels_weight[torch.eq(labels, 2)]))
    else
      acc_bg = 0
    end
    
    if(torch.sum(torch.eq(labels,1)) == 0) then
        agg_fg = 1
    end  
...
function train()
  ...
  print(string.format('Epoch: [%d][TRAINING SUMMARY] Total Time(s): %.2f   loss:%.4f   acc_fg:%.4f   acc_bg:%.4f   reg_acc:%.4f   reg_corr:%.4f', epoch, torch.toc(tic), train_loss, train_fg_acc, train_bg_acc, train_reg_accuracy, train_reg_correct))
  print('\n')
...
function train_batch(roidbs)
    ...
    feval = function(x)
      ...
      for i = 1, #roidbs do
        -- get target
        target_labels = train_batch[2][i]:cuda()
        target_weights = train_batch[3][i]:cuda()
        target_reg = train_batch[4][i]:cuda()
        target_bbox_loss = train_batch[5][i]:cuda()
        
        n_reg = target_labels:size(1) * target_labels:size(2)
                     
        -- batch x (height x width) x boxes
        target_out_l = torch.Tensor(1, target_labels:size(1), target_labels:size(2)):cuda()
        ...
        y_out_l = torch.Tensor(1, y_labels:size(1), y_labels:size(2), y_labels:size(3)):cuda() 
        y_out_l[1] = y_labels
      ...
      reg_correct = reg_correct + correct_boxes
        ...
        table.insert(y, y_out_l:contiguous())
        ...
        -- Calc/add error
        f = f + 10 *sl1:forward(y[2], target[2])/n_reg + log:forward(y[1], target[1]) / n_cls
        local err = criterion:forward(y, target)
        
        -- f = f* (i-1)/i+ err*(1/i)
        
        grad_list = criterion:backward(y, target)
        acc_fg, acc_bg = check_error(target_labels, target_weights, y_out_l)
        ...
        gradient[1][i] = grad_list[1]
        gradient[2][i] = grad_list[2]
  ...
  print(('Epoch: [%d][%d/%d]\tTime(s) %.3f  loss %.4f  LR %.0e AF:%.4f BF %.4f   RA:%.4f  RC:%.4f'):format(
      epoch, batchNumber, math.floor(numOfTrainImages/conf.batch_size), timer:time().real, f, 
      optimState.learningRate, accuracy_fg, accuracy_bg, reg_acc, reg_correct ))
