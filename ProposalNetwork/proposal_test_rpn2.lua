...
function test_batch(test_set, mean, stds)
  ...
  for i = 1, #test_set do
    for j = 1, conf.scales:size(1) do
      ...
      im_blob:resize(1, im_blob:size(1), im_blob:size(2), im_blob:size(3))

      output = model:forward(im_blob:cuda())
      
      -- get labels as bboxes x 2 (pos and neg score)
      labels_prop = output[1][1]:permute(2,3,1)
      ...
      --print(output)
      regression = output[2][1]:permute(2,3,1)
      regression:resize(regression:size(1)*regression:size(2)*regression:size(3)/4, 4)
          
      local im_size = torch.Tensor{im_blob:size(3), im_blob:size(4)}
      ...
      local max, m_idx = torch.max(labels_prop, 2)
